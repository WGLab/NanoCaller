...
def get_data(fname,a=None, b=None, cpu=4,mode='train'):
    t=time.time()
    l=os.stat(fname).st_size
    if mode=='train':
        if a!=None and b!=None:
            my_array=[(fname,x,mode) for x in range(a,b,1000*7406)]
        else:
        ...
        for k in range(iter_steps):
            ...
            for chrom in [2,17]:
                ...
                for i in range(len(tot_list['pos'])-1):
                    ...
                    print(len(x_train))        
                    negative_variants=[get_data(f_path+'neg_%d' %freq,a=tot_list[freq][i],b=tot_list[freq][i+1],cpu=cpu) for freq in [0,5,10,15,20,25]]
