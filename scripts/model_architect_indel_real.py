...
def conv_net(x,weights, biases,keep):
...
def get_tensors(n_input,learning_rate=0):
    ...
    weights = {
        ...
        'wd3': tf.get_variable('W3_3', shape=(f,g), initializer=tf.contrib.layers.xavier_initializer()),
      
        'out': tf.get_variable('W13', shape=(g,3), initializer=tf.contrib.layers.xavier_initializer())
    }
    ...
    biases = {
        ...
        'bd3': tf.get_variable('B3_3', shape=(g), initializer=tf.contrib.layers.xavier_initializer()),
        'out': tf.get_variable('B13', shape=(3), initializer=tf.contrib.layers.xavier_initializer()),
    }
    
    
    x0,x1 = tf.placeholder("float", [None]+n_input), tf.placeholder("float", [None]+n_input),
    
    label_1=tf.placeholder("float", [None, 3])
    label_2=tf.placeholder("float", [None, 3])
    
    
    keep = tf.placeholder(tf.float32)

    score_0 = conv_net(x0, weights, biases, keep)
    score_1 = conv_net(x1, weights, biases, keep)
    ...
    tensors=(x0, x1, label_1,label_2, accuracy, accuracy_0, accuracy_1, cost, optimizer, prob_0,prob_1, keep)
