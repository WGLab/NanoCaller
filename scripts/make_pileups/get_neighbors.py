...
def get_nbr(dct):
    ...
    if dct['type'] in ['redo']:
        ...
        if 'stats' in vcf_path:
            tr_pos={}
            df=pd.read_csv(vcf_path)
            df=df[(df.pos>=start) & (df.pos<end)]
            for i in range(len(df)):
                tr_pos[df.iloc[i].pos]=[df.iloc[i].DP,df.iloc[i].freq,df.iloc[i].ref]
            ...
            for pcol in samfile.pileup(chrom,start-1,end-1,min_base_quality=0, flag_filter=0x4|0x100|0x200|0x400|0x800,truncate=True):
                    if pcol.pos+1 in tr_pos.keys():
                        seq=''.join([x[0] for x in pcol.get_query_sequences( mark_matches=False, mark_ends=False,add_indels=True)]).upper()
                        name=pcol.get_query_names()
                        output.append([pcol.pos+1,tr_pos[pcol.pos+1][0],tr_pos[pcol.pos+1][1],tr_pos[pcol.pos+1][2],seq,':'.join(name)])
            ...
            for rec in bcf_in.fetch(chrom,start,end+1):
                if rec.samples.items()[0][1].get('GT') !=(1,1) and rec.samples.items()[0][1].get('GP')>=6:
