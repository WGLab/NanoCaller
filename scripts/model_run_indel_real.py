...
def genotype_caller_skinny(params,input_type='path',data=None,attempt=0,neg_part='neg.combined'):
    ...
    n_input=params['dims']
    dims=n_input
    chrom_list=list(range(2,20))#[19,20,21]#list(range(2,23)) #params['chrom'].split(':') 
    ...
    (x0, x1, label_1,label_2, accuracy, accuracy_0, accuracy_1, cost, optimizer, prob_0,prob_1, keep)=tensors
    ...
    if params['val']:
        ...
        for v_path in params['test_path'].split(':'):
            ...
            if input_type=='path':
                _, x0_train, x1_train, train_allele_1,train_allele_2= read_pileups_from_file(params['test_path']+'pos', 'train', dims)
                _, nx0_train, nx1_train, ntrain_allele_1,ntrain_allele_2=read_pileups_from_file(params['test_path']+'neg.high', 'train', dims)
    ...
    with tf.Session(config=config)  as sess:
        ...
        for chrom in chrom_list:
                ...
                if input_type=='path':
                    _, x0_train, x1_train, train_allele_1,train_allele_2= read_pileups_from_file(f_path+'pos','train', n_input)

                    _, nx0_train, nx1_train, ntrain_allele_1,ntrain_allele_2=read_pileups_from_file(f_path+'neg.combined', 'train', n_input)
            ...
            for chrom in chrom_list:
                ...
                for i in range(iters):
                    ...
                    for batch in range(int(np.ceil(len(x0_train)/batch_size))):
                        ...
                        opt,loss = sess.run([optimizer,cost], feed_dict={x0: batch_x0, x1: batch_x1, label_1:batch_allele_1, label_2:batch_allele_2, keep:0.5})
                ...
                for batch in range(int(np.ceil(len(x0_train)/batch_size))):
                        ...
                        batch_loss,batch_acc= sess.run([cost, accuracy], feed_dict={x0: batch_x0, x1: batch_x1, label_1:batch_allele_1, label_2:batch_allele_2, keep:1.0})
                ...
                if params['val'] and (k<3 or chrom==chrom_list[-1]):
                    ...
                    for val in val_list:
                        ...
                        for batch in range(int(np.ceil(len(vx0_test)/(batch_size)))):
                            ...
                            batch_loss,batch_acc= sess.run([cost, accuracy], feed_dict={x0: vbatch_x0, x1: vbatch_x1, label_1: vbatch_allele_1, label_2: vbatch_allele_2, keep:1.0})
    ...
    with open(vcf_path,'w') as f:
        ...
        for i in range(len(tmp_sz)-1):
            ...
            for batch in range(int(np.ceil(len(x0_test)/batch_size))):
                ...
                batch_prob_0,batch_prob_1= sess.run([prob_0,prob_1], feed_dict={x0: batch_x0, x1: batch_x1, keep:1.0})
                ...
                for j in range(len(batch_pred_0)):
                    ...
                    if batch_pred_0[j]!=0 or batch_pred_1[j]!=0:
                        ...
                        if batch_pred_0[j]!=0 and batch_pred_1[j]!=0:
                            q3=(q0+q1)/2
                            
                        elif batch_pred_0[j]!=0:
                            q3=q0
                            
                        elif batch_pred_1[j]!=0:
                            q3=q1
                            
                        
                        s='%s,%d,%s,%s,%.4f,%.4f,%.4f,%.4f\n' %(chrom, batch_pos[j], rev_allele_map[batch_pred_0[j]], rev_allele_map[batch_pred_1[j]],q0,q1,(q0+q1)/2,q3)
                        f.write(s)
